...
ReadNumber:
    ...
    cmp al, '-'        ;compara se é um sinal negativo
    je near EhNegativo ;se for, chama a rotina de leitura de sinal negativo
    ...
    Fim:
        cmp eax,1      ;compara se o valor de eax eh 1
        je near MultiplicaPorMenosUm        ;se for, chama a rotina de multiplicacao por -1
        ret

EhNegativo:
    mov eax, 1       ;atribui 1 ao valor de eax
    call ReadNumber  ;chama a rotina de leitura do teclado

MultiplicaPorMenosUm:
    mov al, -1     ;atribui -1 ao valor de al
    mov bl, [di]   ;pega o valor de di
    imul bl        ;multiplica o valor de di por -1
    mov [di],ax    ;atualiza o valor de di
...
Soma:
    ...
    or dx, 0            ;verifica se o resultado eh maior que 255
    js near label1      ;se for, chama a rotina de impressao de resultado maior que 255
...
Multi:
    ...
    mov dx,[result]     ;pega o valor de result
    or dx, 0            ;verifica se o resultado eh maior que 255
    js label1           ;se for, chama a rotina de impressao de resultado maior que 255
...
label1:
    mov [di], dx        ;atualiza o valor de di
    mov al, -1          ;atribui -1 ao valor de al
    mov bl, [di]        ;pega o valor de di
    imul bl             ;multiplica o valor de di por -1
    mov [di],ax         ;atualiza o valor de di

    mov dx,sinal        ;imprime o sinal
    mov ah,9h           ;chama a rotina de impressao
    int 21h
...
Divisao:
    ...
    idiv bl              ;divide o valor de num1 por num2
...
segment data
    ...
    resto dw 0          ;resto
    ehnegativo1 dw 0     ;eh negativo 1
    ehnegativo2 dw 0     ;eh negativo 2
    teste db 10,13 , "TESTE $"
    teste2 db 10,13 , "TESTE 2 $"
    sinal db 10,13 , "-$"
