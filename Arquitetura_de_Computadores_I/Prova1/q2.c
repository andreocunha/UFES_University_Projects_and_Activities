...
float a[] = {0.756, -1.567}; 	// declara um vetor de float (PF single) chamado a, com 2 posições.
				// O vetor a é inicializado com 2 números reais 0.ZYX e -1.YXZ
float b[] = {-2.675, 3.657};	// declara um vetor de float (PF single) chamado b, com 2 posições.
...
int main() {		// Início do programa (seção .text)
    for (int i=0; i<dim; i++) {	// Declara um loop que começa com valor i=0 (i é inteiro, 32 bits) e
         ...
         r[i] = a[i] + b[i];
         printf("%f\n", r[i]);		// Soma os valores nas posições i (i=0 e i=1) dos vetores a e b (ou 
